...
impl CardValue for CamelHandValue<'_> {
        ...
        match (a, b) {
            (Some(a), Some(b)) => {
                let orders = a
                    ...
                    .zip(b.chars().take(5))
                    .flat_map(|(a, b)| {
                        let a_value = CamelHandValue::get_char_as_card(a);
                        let b_value = CamelHandValue::get_char_as_card(b);
...
impl Ord for CamelHandValue<'_> {
    fn cmp(&self, other: &Self) -> Ordering {
        match self {
            Five(s) => CamelHandValue::compare_values(Some(s), other.five_value()),
            Four(s) => match other {
                Five(_) => Ordering::Less,
                _ => CamelHandValue::compare_values(Some(s), other.four_value()),
            },
            Full(s) => match other {
                Five(_) | Four(_) => Ordering::Less,
                _ => CamelHandValue::compare_values(Some(s), other.full_values()),
            },
            Three(s) => match other {
                Five(_) | Four(_) | Full(_) => Ordering::Less,
                _ => CamelHandValue::compare_values(Some(s), other.three_value()),
            },
            TwoPair(s) => match other {
                Five(_) | Four(_) | Full(_) | Three(_) => Ordering::Less,
                _ => CamelHandValue::compare_values(Some(s), other.two_pair_value()),
            },
            Pair(s) => match other {
                Five(_) | Four(_) | Full(_) | Three(_) | TwoPair(_) => Ordering::Less,
                _ => CamelHandValue::compare_values(Some(s), other.pair_value()),
            },
            HighCard(s) => match other {
                Five(_) | Four(_) | Full(_) | Three(_) | TwoPair(_) | Pair(_) => Ordering::Less,
                _ => CamelHandValue::compare_values(Some(s), other.high_card_value()),
            },
        }
    }
}
